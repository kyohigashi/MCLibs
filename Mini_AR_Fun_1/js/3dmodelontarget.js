var World = {
    ...
    createOverlays: function createOverlaysFn() {
        ...
        */
        var dayModel = new AR.Model("assets/ICC_happybirthday_1031_DAY.wt3", {
            scale: {
                x: 0.014,
                y: 0.014,
                z: 0.014
        ...
        var dayAnimation1 = new AR.ModelAnimation(dayModel, "happy_birthday5_animation");


        var nightModel = new AR.Model("assets/ICC_happybirthday_1031_NIGHT.wt3", {
            scale: {
                x: 0.014,
                y: 0.014,
                z: 0.014
        ...
        var nightAnimation2 = new AR.ModelAnimation(nightModel, "Night_set_animation");
        /*
        	Similar to 2D content the 3D model is added to the drawables.cam property of an AR.Trackable2DObject.
        */
        var trackable = new AR.Trackable2DObject(this.tracker, "Small-ICC-chop", {
            drawables: {
                cam: [dayModel]
            },
        ...
        var trackable = new AR.Trackable2DObject(this.tracker, "Big-ICC-chop", {
            drawables: {
                cam: [nightModel]
            },
            onEnterFieldOfVision: function() {
                if (nightAnimation2.isRunning()) {
                    nightAnimation2.resume();
                } else {
                    window.setTimeout(function() {
                        nightAnimation2.start(-1);
                    }, 500);
                }
                if (nightAnimation1.isRunning()) {
                    nightAnimation1.resume();
                } else {
                    window.setTimeout(function() {
                        nightAnimation1.start(-1);
                    }, 500);
                }
            }
        });
